...
if "Horário" in df.columns and df["Horário"].notna().any(): df_tt=df.sort_values("Horário",ascending=False).copy()
else: df_tt=df.copy()
col_ctrl1,col_ctrl2=st.columns([2,1])
with col_ctrl1: n_linhas=st.slider("Nº de registros exibidos:",min_value=10,max_value=200,value=30,step=10,key="tt_slider")
with col_ctrl2:
    csv_data=df_tt.to_csv(index=False).encode("utf-8"); st.write("<br>",unsafe_allow_html=True)
    st.download_button(label="📥 Baixar Histórico Completo",data=csv_data,file_name="times_and_trade_completo.csv",mime="text/csv")
...
for _,r in df_tt.head(n_linhas).iterrows():
    horario=(r["Horário"].strftime("%H:%M:%S") if pd.notna(r.get("Horário")) and isinstance(r.get("Horário"),pd.Timestamp) else "—")
    rows_html+=(f"<tr><td style='color:#4a7a75;font-size:13px;'>{horario}</td><td class='text-gold' style='font-weight:bold;font-size:13px;'>{r.get('Strike','—')}</td><td style='font-size:13px;'>{_tt_side_html(r.get('Lado','—'))}</td><td style='color:#ccddf8;font-size:13px;'>{int(r.get('Vol',0)):,}</td><td style='color:#fa0;font-size:13px;'>${r.get('Fin_Opc',0):,.0f}</td><td style='font-size:13px;'>{_tt_mm_html(r.get('Acao_MM','—'))}</td><td style='font-size:13px;'>{_tt_flow_html(r.get('d_flow',0))}</td><td style='font-size:13px;'>{_tt_flow_html(r.get('g_flow',0))}</td><td style='font-size:13px;'>{_tt_flow_html(r.get('v_flow',0))}</td></tr>")

st.markdown(f"<div class='dashboard-card' style='padding:16px;overflow-x:auto;'><table class='tt-table'><thead><tr><th>HORÁRIO</th><th>STRIKE</th><th>LADO</th><th>VOLUME</th><th>FINANCEIRO</th><th>AÇÃO MM</th><th>DELTA $</th><th>GAMMA $</th><th>VANNA $</th></tr></thead><tbody>{rows_html}</tbody></table></div>", unsafe_allow_html=True)
