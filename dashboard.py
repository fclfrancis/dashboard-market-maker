...
with st.sidebar:
    ...
    dir_path = st.text_input(
        ...
        value=r"C:\Dados\Barchart",
        help="Cole aqui o caminho completo da pasta onde os JSONs estão salvos.",
    )

    arquivos_encontrados = []
    if os.path.exists(dir_path):
        arquivos_encontrados = glob.glob(os.path.join(dir_path, "*.json"))
        if arquivos_encontrados:
            ...
            st.info(f"✅ {len(nomes_arquivos)} arquivos encontrados.")
            with st.expander("📄 Ver arquivos na pasta"):
                st.write(nomes_arquivos)
        else:
            st.warning("⚠ Nenhum arquivo .json encontrado nesta pasta.")
    else:
        st.error("❌ Caminho de diretório inválido.")

    st.markdown("<div class='glow-divider'></div>", unsafe_allow_html=True)

    snapshots = {}
    if arquivos_encontrados:
        snapshots = agrupar_snapshots(arquivos_encontrados)

    if snapshots:
        momento = st.selectbox("📅 Snapshot (data):", list(snapshots.keys()))
    else:
        momento = None
        st.info("Aguardando leitura de diretório...")
    ...
    if _sb_key not in st.session_state and arquivos_encontrados and momento and snapshots.get(momento):
        _frames_sb = [parse_json(a) for a in snapshots[momento]]
        _frames_sb = [f for f in _frames_sb if f is not None and not f.empty]
        if _frames_sb:
...
if not momento:
    st.markdown(alert_box("⬆ Informe o caminho da pasta na sidebar para iniciar a análise.", "info"),
                unsafe_allow_html=True)
    st.stop()
...
frames    = [parse_json(arq) for arq in lista_arq]
